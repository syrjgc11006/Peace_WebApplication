...
namespace Peace.Com.Mvc
{
    ...
    public static class HtmlHelperPaginationExtensions
    {
        ...
        public static MvcHtmlString PagingButton(this HtmlHelper html, Pager  pagingDataSet, PaginationMode paginationMode = PaginationMode.NumericNextPrevious, int numericPagingButtonCount = 7)
        {
        ...
        /// <param name="numericPagingButtonCount">数字分页按钮显示个数</param>
        /// <returns>分页按钮html代码</returns>
        public static MvcHtmlString AjaxPagingButton(this HtmlHelper html, Pager pagingDataSet, string updateTargetId, PaginationMode paginationMode = PaginationMode.NumericNextPrevious, int numericPagingButtonCount = 7, string ajaxUrl = null)
        {
        ...
        /// <returns>分页按钮html代码</returns>
        private static MvcHtmlString PagingButton(this HtmlHelper html, Pager pagingDataSet, bool enableAjax, string updateTargetId, PaginationMode paginationMode = PaginationMode.NumericNextPrevious, int numericPagingButtonCount = 7, string ajaxUrl = null)
        {
            if (pagingDataSet.RecordCount == 0 || pagingDataSet.PageSize == 0)
                return MvcHtmlString.Empty;

            //计算总页数
            int totalPages = (int)(pagingDataSet.RecordCount / (long)pagingDataSet.PageSize);
            if ((pagingDataSet.RecordCount % pagingDataSet.PageSize) > 0)
                totalPages++;
            ...
            string pagingContainer = "<div class=\"default\"";
            if (enableAjax)
                pagingContainer += " plugin=\"ajaxPagingButton\" data=\"" + HttpUtility.HtmlEncode(JsonConvert.SerializeObject(new { updateTargetId = updateTargetId })) + "\"";
            pagingContainer += ">";
                    ...
                    for (int i = 1; i < startNumericPageIndex; i++)
                    {
                        ...
                        else
                        {
                            if (pagingDataSet.PageIndex == i)
                                pagingButtonsHtml.AppendFormat("<span class=\"current\">{0}</span>", i);
                            else
